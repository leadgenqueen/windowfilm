  ...
  return <section className="py-20 bg-gradient-warm">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {reviews.slice(0, 3).map((review, index) => <Card key={index} className="bg-card shadow-card border-0 hover:shadow-premium transition-all duration-300">
              <CardContent className="p-6">
                ...
                <div className="flex space-x-1 mb-3">
                  {[...Array(review.rating)].map((_, i) => <Star key={i} className="h-4 w-4 fill-trust-gold text-trust-gold" />)}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          {reviews.slice(3, 5).map((review, index) => <Card key={index + 3} className="bg-card shadow-card border-0 hover:shadow-premium transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center mb-4">
                  <div className="w-12 h-12 bg-coastal-blue text-white rounded-full flex items-center justify-center font-semibold mr-4">
                    {review.avatar}
